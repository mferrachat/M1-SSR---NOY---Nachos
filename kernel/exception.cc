      ...
      switch(type) {
        ...
        case SC_CLOSE: {
          ...
          case SC_SEM_CREATE: {
            ...
            int initialValue = g_machine -> ReadIntRegister(5);
            // create semaphore
            DEBUG('e', (char*)"SC_SEM_CREATE : name = %s, value = %d\n", debugName, initialValue);
            Semaphore *sema = new Semaphore(debugName, initialValue);
            if (sema) {
              // return semaphore ID
              DEBUG('e', (char*)"Sémaphore créé\n");
              int32_t sid = g_object_ids -> AddObject(sema);
              g_machine -> WriteIntRegister(2,sid);
              g_syscall_error -> SetMsg((char*)"",NO_ERROR);
            } else {
              DEBUG('e', (char*)"Sémaphore %s non créé\n", debugName);
              g_machine -> WriteIntRegister(2,ERROR);
              sprintf(msg,"%s",debugName);
              g_syscall_error -> SetMsg(msg,OUT_OF_MEMORY);
            }
          ...
          case SC_LOCK_ACQUIRE: {
            ...
            // Get the lock ID
            int32_t lid = g_machine -> ReadIntRegister(4);
          ...
          case SC_COND_DESTROY: {
            ...
            // get the cond id
            int32_t cid = g_machine -> ReadIntRegister(4);
